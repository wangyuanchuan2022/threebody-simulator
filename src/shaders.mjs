...
vec3 atmosphere(vec3 rd, bool disk) {
  ...
  float night=exp(-daylight*24.0);
  sum+=vec3(.0008,.0015,.0026)*night;
  if(disk && rd.y>0.0) {
    vec2 st=vec2(atan(rd.z,rd.x)*220.0,asin(rd.y)*220.0);
    vec2 cell=floor(st);float h=hash(cell);
    float star=pow(max(0.0,1.0-length(fract(st)-.5)*2.0),18.0)*step(.996,h);
    float band=pow(max(0.0,1.0-abs(rd.x*.5+rd.y*.7+rd.z*.2)),12.0);
    sum+=night*(vec3(.35,.43,.55)*star+vec3(.001,.0012,.0017)*fbm(rd.xz*100.0)*band);
  }
  return sum;
}
